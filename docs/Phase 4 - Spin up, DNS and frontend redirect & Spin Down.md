...
At this point, once the ec2's are ready, the dynamoDB row should look something like this:

| **Field**           | **Value**                  |
| ------------------- | -------------------------- |
| **id**              | `0swis5jucu0...`           |
| **status**          | `DNS_WRITTEN`              |
| **created_at**      | `2025-12-07T20:52:31.2909` |
| **meet_time**       | `2025-12-07T...`           |
| **jitsi_url**       | `https://meet...`          |
| **teacher_name**    | `Chris`                    |
| **teacher_ec2_id**  | `i-06444d5d5...`           |
| **teacher_ec2_url** | `https://i-064...`         |
| **student_name**    | `Boonki Goongo`            |
| **student_ec2_id**  | `i-0e85121c0...`           |
| **student_ec2_url** | `https://i-0e8...`         |
...
![[Screenshot 2025-12-07 at 4.36.47 PM.png]]
...
- This is basically the entire safety net logic statement. It will kill the ec2 instances in the following situations at the 15-minute mark:
	- If both teacher and student data are empty AND the meeting doesn't say IN_PROGRESS, then we assume that nobody showed up
	- If only one person has data written, someone showed up and waited for the other person, but they never showed up
	- Both student and data have data written, suggesting a very short meeting.
...
if should_kill is set to true by any of the logic statements, then we proceed with terminating all instances using their id's.

Warning: Note that I came up with this safety net logic as a proposed solution to some user situations. In a perfect situation, if compute and cost wasn't constrained, maybe a solution would be to ask the teacher, "how long will your meeting be"? And give them a number of hours per month. That way, you can just have it on a timer. It's up to you.
### IAM Role
